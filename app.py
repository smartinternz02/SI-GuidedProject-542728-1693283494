...
app = Flask(__name__)
app.secret_key="ab+5"
conn=ibm_db.connect("DATABASE=bludb;HOSTNAME=6667d8e9-9d4d-4ccb-ba32-21da3bb5aafc.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;PORT=30376;UID=hdt83722;PWD=CVeLV1yWvbcmEJHy;SECURITY=SSL;SSLCERTIFICATE=DigiCertGlobalRootCA.crt",'','' )
...
def login():
    if request.method == "POST":
        ...
        print(uname,pword)
        sql = "SELECT * FROM REGISTER WHERE USERNAMR= ?  AND PASSWORD= ?"
...
def register():
     if request.method == 'POST':
         ...
         sql = "SELECT * FROM REGISTER WHERE USERNAMR =?" 
         stmt=ibm_db.prepare(conn,sql)
         ibm_db.bind_param(stmt, 1, uname)
         ibm_db.execute(stmt)
         out=ibm_db.fetch_assoc(stmt)
